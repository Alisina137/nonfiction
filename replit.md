...
| `GROQ_API_KEY` | At least 1 AI key | Groq Llama 3.3 70B — fast LPU fallback (#2) |
| `CEREBRAS_API_KEY` | At least 1 AI key | Cerebras Inference — low-latency fallback (#3) |
| `OPENROUTER_API_KEY` | At least 1 AI key | OpenRouter (DeepSeek R1, Qwen 3, Llama 4 Maverick) (#4) |
| `SAMBANOVA_API_KEY` | At least 1 AI key | SambaNova Cloud — last-resort fallback (#5) |
...
- **Multi-model specialized pipeline**: Each of the 6 book-creation phases uses the model best suited for that task type. `TASK_CHAINS` in `aiRouter.ts` defines the per-phase provider/model order. `CONTENT_TYPE_TO_TASK` in `index.ts` routes each API call to the right phase with zero change to individual route code.
  - **idea** (titles/subtitles): Gemini Flash → Groq → Cerebras → OpenRouter → SambaNova
  - **research** (competitor analysis): DeepSeek R1 → Gemini Pro → Groq → Cerebras → SambaNova
  - **outline** (structure/chapters): Gemini Pro → DeepSeek R1 → Groq → Cerebras → SambaNova
  - **write** (prose content): Gemini Pro → OpenRouter (DeepSeek/Qwen/Maverick) → SambaNova → Cerebras → Groq
  - **edit** (improvement): Llama 4 Maverick → Gemini Pro → Groq → Cerebras → SambaNova
  - **metadata** (description/SEO): Gemini Flash-Lite → Groq → Cerebras → OpenRouter → SambaNova
- **5-provider fallback chain**: Gemini → Groq → Cerebras → OpenRouter → SambaNova. Each provider is independently disabled for 10min/60min/24h on rate-limit/hard/quota errors. This maximises uptime on free-tier keys.
