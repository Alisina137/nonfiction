...
router.post("/multi-search", async (req, res) => {
  ...
  // ── Expand if fewer than 20 unique books using real public metadata ────────
  if (deduped.length < 20) {
    console.log(`[multi-search] Only ${deduped.length} books found — running Open Library expansion`);
    const broadQuery = validKeywords.slice(0, 3).join(" ");
    try {
      const extraBooks = await openLibrarySearch(broadQuery, 20);
      const extraEntries = extraBooks.map((b: any) => ({ book: b, matchedKeywords: [broadQuery] }));
      const combined = deduplicateBooks([...allEntries, ...extraEntries]);
      if (combined.length > deduped.length) {
        deduped = combined;
        console.log(`[multi-search] After Open Library expansion: ${deduped.length}`);
      }
    } catch (e: any) {
      console.warn("[multi-search] Open Library expansion failed:", e.message);
    }
  }
...
router.post("/amazon-search", async (req, res) => {
  ...
  // ── 3. Open Library fallback — real metadata, never AI-invented books ────────
