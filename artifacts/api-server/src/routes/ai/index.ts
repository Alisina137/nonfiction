...
router.post("/generate-chapters", async (req, res) => {
  try {
    ...
    const wa = Number(parts[0]), wb = Number(parts[1]);
    if (Number.isFinite(wa) && Number.isFinite(wb)) targetWords = ((wa + wb) / 2) * 1000;
    ...
    async function attempt() {
      const { text, usedProvider, exhaustedProviders } = await generateContent(
        prompt,
        undefined,
        aiOptsFromReq(req, TOKEN_LIMITS.generateChapters)
      );
      setProviderHeader(res, usedProvider, exhaustedProviders);
      return { text, usedProvider };
    }
...
/** POST /api/ai/regenerate-chapter — regenerate a single chapter preserving the rest */
router.post("/regenerate-chapter", async (req, res) => {
  try {
    const {
      project, chapterIndex, totalChapters,
      currentWords, existingChapterTitles,
      prevChapterTitle, nextChapterTitle
    } = req.body || {};
    ...
    const ctx = {
      title:        safeStr(bd.title || research.bookTitle || project.bookTitle?.selectedCard?.title),
      bookTopic:    safeStr(research.bookTopic),
      niche:        [research.mainNicheLabel, research.subNicheLabel].filter(Boolean).join(" › "),
      audience:     safeStr(bd.audience || pb.proposedAudience || research.targetAudience || intel.targetAudience),
      tone:         safeStr(bd.tone || pb.proposedTone || intel.energyStyle),
      targetWords,
      corePromise:  safeStr(bd.corePromise || pb.bookPitch),
      chapterIndex: Number(chapterIndex) || 0,
      totalChapters: Number(totalChapters) || 1,
      currentWords:  Number(currentWords) || 3000,
      prevChapterTitle: safeStr(prevChapterTitle),
      nextChapterTitle: safeStr(nextChapterTitle),
      existingChapterTitles: Array.isArray(existingChapterTitles)
        ? existingChapterTitles.map((t: any) => safeStr(t)).filter(Boolean)
        : [],
    };

    const prompt = regenerateChapterPrompt(ctx);
    const { text, usedProvider, exhaustedProviders } = await generateContent(
      prompt,
      undefined,
      aiOptsFromReq(req, TOKEN_LIMITS.chapterStrategy)
    );
    setProviderHeader(res, usedProvider, exhaustedProviders);
