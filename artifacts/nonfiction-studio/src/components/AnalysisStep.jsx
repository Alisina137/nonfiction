...
export default function AnalysisStep({ research, analysis, errors, updateAnalysis, patchBook, removeBook }) {
  ...
  async function runAmazonSearch(mode) {
    ...
    try {
      ...
      if (data.needsApiKey) { setLocalMsg(data.message || "Configure SCALE_SERP_API_KEY on the server."); return; }
  ...
  async function expandBook(book) {
    ...
    try {
      ...
      if (data.needsApiKey) { setLocalMsg(data.message || "Add SCALE_SERP_API_KEY to load details."); return; }
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <p className="w-full text-xs text-slate-500">
            ...
            <span className="font-mono">SCALE_SERP_API_KEY</span>.
          </p>
        </div>
      </div>
