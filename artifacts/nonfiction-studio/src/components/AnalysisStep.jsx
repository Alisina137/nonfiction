    ...
    try {
      const res = await fetch("/api/ai/final-positioning", {
        ...
        body: JSON.stringify({
          ...
          painPoints:        intelligence.readerPainProfile     || "",
          desiredOutcomes:   intelligence.transformationPromise || "",
          buyerIntent:       intelligence.positioningStrategy   || "",
          marketOpportunity: intelligence.marketGapAnalysis     || "",
          competitiveInsights: intelligence.bestsellerDNA       || "",
          readerMotivation:  (intelligence.emotionalTriggers || []).join(", "),
          tone:              intelligence.toneRecommendation    || "",
      ...
      <div className="mt-10 rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50/60 via-white to-teal-50/40 p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-slate-600">
              Combines your Research inputs and competitor intelligence to generate the strongest possible Amazon KDP title, subtitle, and topic. Run this after Analyze Competitors.
            </p>
          </div>
          <button
            type="button"
        ...
        {posError && (
          <p className="mt-3 text-sm font-medium text-rose-700">{posError}</p>
        )}
                    ...
                    return (
                      <li key={i}>
                        ...
                        >
                          {opt.subtitle}
                          {opt.reason && (
                            <span className={`block mt-1 text-[10px] italic ${active ? "text-emerald-100" : "text-slate-400"}`}>
                              {opt.reason}
                            </span>
                          )}
                        </button>
