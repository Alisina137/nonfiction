...
export function buildBookContext(project) {
  ...
  const ctx = {
    ...
    // ── Blueprint Intelligence (generated by the Blueprint step) ──────────
    // These flow downstream to every AI generation call as the "strategic brain"
    blueprintMission:      cap(bd.blueprintLayers?.bookIdentity?.bookMission, 160),
    blueprintTeachingStyle: cap(bd.blueprintLayers?.bookIdentity?.teachingStyle, 120),
    blueprintEvidenceStyle: cap(bd.blueprintLayers?.bookIdentity?.evidenceStyle, 120),
    blueprintFrameworkStyle: cap(bd.blueprintLayers?.bookIdentity?.frameworkStyle, 80),
    blueprintPracticality:  cap(bd.blueprintLayers?.bookIdentity?.practicalityLevel, 60),
    blueprintPersonality:   cap(bd.blueprintLayers?.bookIdentity?.bookPersonality, 80),
    blueprintCoreFramework: cap(bd.blueprintLayers?.bookStrategy?.coreFramework, 180),
    blueprintLearningProg:  cap(bd.blueprintLayers?.bookStrategy?.learningProgression, 200),
    blueprintKeyVocabulary: Array.isArray(bd.blueprintLayers?.bookStrategy?.keyVocabulary)
      ? bd.blueprintLayers.bookStrategy.keyVocabulary.slice(0, 6).join(", ")
      : "",
    blueprintRecurringConcepts: Array.isArray(bd.blueprintLayers?.bookStrategy?.recurringConcepts)
      ? bd.blueprintLayers.bookStrategy.recurringConcepts.slice(0, 4).join(", ")
      : "",
    blueprintTransformationGoal: cap(bd.blueprintLayers?.qualityTargets?.transformationGoal, 180),
    blueprintEmotionalGoal:      cap(bd.blueprintLayers?.qualityTargets?.emotionalGoal, 120),
    blueprintOriginalityGoal:    cap(bd.blueprintLayers?.qualityTargets?.originalityGoal, 160),
    blueprintReaderCurrentSituation: cap(bd.blueprintLayers?.readerModel?.currentSituation, 160),
    blueprintReaderDesiredFuture:    cap(bd.blueprintLayers?.readerModel?.desiredFuture, 160),
    blueprintFalseBeliefsToBreak: Array.isArray(bd.blueprintLayers?.readerModel?.falseBeliefsToBreak)
      ? bd.blueprintLayers.readerModel.falseBeliefsToBreak.slice(0, 3).join("; ")
      : "",
    blueprintMarketGaps: Array.isArray(bd.blueprintLayers?.marketModel?.marketGaps)
      ? bd.blueprintLayers.marketModel.marketGaps.slice(0, 3).join("; ")
      : "",
    blueprintCompetitiveAdvantages: Array.isArray(bd.blueprintLayers?.marketModel?.competitiveAdvantages)
      ? bd.blueprintLayers.marketModel.competitiveAdvantages.slice(0, 3).join("; ")
      : "",
    blueprintTransformationMap: Array.isArray(bd.transformationMap)
      ? bd.transformationMap.map(s => `${s.stage}: ${s.description || ""}`).join(" → ").slice(0, 400)
      : "",
