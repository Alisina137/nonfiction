...
**Residual flakiness:** Even with sanitize+repair, a small fraction of responses still fail to parse (e.g. a provider stops generation mid-string with no closing quote/brace in an unrecoverable spot). This caused intermittent 500s on `back-matter/appendix-entry`, `back-matter/key-lessons`, and `back-matter/glossary` — the "Generate" button in Write Step > Back Matter would silently fail with a generic error and no entry created. Fix: any route whose success depends on `extractJSON` succeeding should retry the whole AI call once (new prompt round-trip, not just re-parsing) before surfacing an error — see `runLongJSON()` helper in `artifacts/api-server/src/routes/ai/index.ts`. Prefer wrapping new JSON-returning long-form routes with `runLongJSON` instead of raw `runLong` + `extractJSON`.
